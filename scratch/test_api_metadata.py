...
def test_metadata_with_event():
    print("=== STARTING API TEST: MEMORY RETRIEVAL ===")
    
    # 1. Trigger a high-severity supplier delay event
    event_payload = {
        "type": "supplier_delay",
        "severity": 0.95,
        "source": "Manual Test",
    ...
    # 2. Get the latest state and check metadata
    print("\n2. Fetching current control tower state to check metadata...")
    # Wait a moment for the background processing if any (though usually sync in this setup)
    time.sleep(1) 
    
    state_response = requests.get(f"{BASE_URL}/control-tower/summary")
    ...
    if referenced_cases:
        ...
        for case in referenced_cases:
            print(f" - ID: {case['case_id']} | Type: {case['event_type']} | Similarity: {case.get('similarity_score', 'N/A')}")
    else:
...
if __name__ == "__main__":
    try:
        ...
        print(f"Connection Error: {e}. Make sure the uvicorn server is running on port 8000.")
